...
To validate the generated code, we implemented a  built-in Inverse Kinematics (IK) solver to simulate basic robot's movements.
Application support various robot models, including popular industrial robot arms like ABB, AUBO and others models in future. while build-in simulator allows user to test environment setup and robot's movements it lack of advanced physics simulation which is crucial for realistic movement and interaction validation.
Ros and Gazebo provide a more comprehensive simulation environment with accurate physics modeling, allowing us to validate the generated code in a realistic setting. We integrated our application with ROS and Gazebo and allow seamless workflow between code generation and simulation validation.
This setup allows us to test the functionality and performance of the generated code without the need for physical hardware.
\begin{figure}[ht]
  \centering
